...
class GraphChart2:
    ...
    def drawChart(self):
        ...
        # Javier changed fromstring to frombytes
        #self.img.fromstring(wxImag.GetData())
        self.img.frombytes(bytes(wxImag.GetData()))
        ...
        # Javier changed deprecated EmptyImage to Image instead
        #wxImg = wx.EmptyImage(self.img.size[0], self.img.size[1])
        wxImg = wx.Image(self.img.size[0], self.img.size[1])
        # Javier changed from tostring to tobytes
        #wxImg.SetData(self.img.tostring())
        wxImg.SetData(self.img.tobytes())
        # Javier changed deprecated BitmapFromImage to Bitmap instead
        #self.buffer = wx.BitmapFromImage(wxImg)
    ...
    def drawHouses(self, chouses, r1, r2):
        ...
        for i in range (1, houses.Houses.HOUSE_NUM+1):
            ...
            x1 = int(cx+math.cos(math.pi+dif)*r1)
            y1 = int(cy+math.sin(math.pi+dif)*r1)
            x2 = int(cx+math.cos(math.pi+dif)*r2)
            y2 = int(cy+math.sin(math.pi+dif)*r2)
    ...
    def drawAscMC(self, ascmc, r1, r2, rArrow):
        ...
        for i in range(4):
            ...
            x1 = int(cx+math.cos(ang)*r1)
            y1 = int(cy+math.sin(ang)*r1)
            x2 = int(cx+math.cos(ang)*r2comma)
            y2 = int(cy+math.sin(ang)*r2comma)
            self.bdc.DrawLine(x1, y1, x2, y2)
...
#       self.bdc.FloodFill(x, y, clr, wx.FLOOD_BORDER)
    ...
    def drawAscMCPos(self):
        ...
        for i in range(2):
            ...
            wdeg, hdeg = get_size(self.fntText, degtxt)
            ...
            wdeg, hdeg = get_size(self.fntSmallText2, mintxt)
...
#                           t = 'R'
    ...
    def drawTermsLines(self):
        ...
        for i in range(num):
            ...
            for j in range(subnum):
                ...
                x1 = int(cx+math.cos(math.pi+shift-math.radians(deg))*self.rTerms)
                y1 = int(cy+math.sin(math.pi+shift-math.radians(deg))*self.rTerms)
                x2 = int(cx+math.cos(math.pi+shift-math.radians(deg))*self.rDecans)
                y2 = int(cy+math.sin(math.pi+shift-math.radians(deg))*self.rDecans)
    ...
    def drawDecansLines(self):
        ...
        i = math.pi+math.radians(shift)
        while i>-math.pi+math.radians(shift):
            x1 = int(cx+math.cos(i)*self.rInner)
            y1 = int(cy+math.sin(i)*self.rInner)
            x2 = int(cx+math.cos(i)*self.rDecans)
            y2 = int(cy+math.sin(i)*self.rDecans)

    ...
    def printTransits(self, ls):
        ...
        for tr in ls:
            ...
            if tr.objtype == Transit.PLANET:
                ...
                print(f"day {tr.day}: {planets[tr.plt]} {aspc[tr.aspect]} {planets[tr.obj]} house:{tr.house+1} {d}:{m:02d}:{s:02d}")
                ...
                print(f"day {tr.day}: {planet[tr.plt]} {asps[tr.aspect]} {ascmc[tr.obj]} house:{tr.house+1} {d}:{m:02d}:{s:02d}")
            else:
                #print 'day %d: %s %s house:%d %d:%02d:%02d' % (tr.day, planets[tr.plt], signs[tr.obj], tr.house+1, d, m, s)
                print(f"day {tr.day}: {planets[tr.plt]} {signs[tr.obj]} house:{tr.house+1} {d}:{m:02d}:{s:02d}")

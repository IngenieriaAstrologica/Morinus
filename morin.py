...
# Roberto Size changed  V 7.X.X
        wx.Frame.__init__(self, parent, id, title, wx.DefaultPosition, wx.Size(650, 453))
# ###########################################
        self.fpath = ''
...
# Roberto change  V 7.3.0
        ...
        self.ID_Antiscia, self.ID_Dodecatemoria, self.ID_Aspects, self.ID_Midpoints, self.ID_RiseSet, self.ID_Speeds, self.ID_ZodPars, self.ID_FixStars, self.ID_FixStarsAsps,
        self.ID_Arabians, self.ID_Strip, self.ID_PlanetaryHours, self.ID_ExactTransits, self.ID_Profections, self.ID_CustomerSpeculum, self.ID_Firdaria, self.ID_PrimaryDirs) = range(111,135)
...
# ###########################################
        ...
        #Help-menu
        self.mhelp.Append(self.ID_SpecialEdition, mtexts.menutxts['HEMSpecialEdition'], mtexts.menutxts['HEMSpecialEditionDoc'])
        self.mhelp.AppendSeparator()
        self.mhelp.Append(self.ID_Campus, mtexts.menutxts['HEMCampus'], mtexts.menutxts['HEMCampusDoc'])
        self.mhelp.Append(self.ID_Libros, mtexts.menutxts['HEMLibros'], mtexts.menutxts['HEMLibrosDoc'])
        self.mhelp.Append(self.ID_Aprende, mtexts.menutxts['HEMAprende'], mtexts.menutxts['HEMAprendeDoc'])
        self.mhelp.AppendSeparator()
...
# ###########################################
        ...
        self.Bind(wx.EVT_MENU, self.onSpecialEdition, id=self.ID_SpecialEdition)
        self.Bind(wx.EVT_MENU, self.onCampus, id=self.ID_Campus)
        self.Bind(wx.EVT_MENU, self.onLibros, id=self.ID_Libros)
        self.Bind(wx.EVT_MENU, self.onAprende, id=self.ID_Aprende)
...
# ###########################################
    ...
    def onSpecialEdition(self, event):
        #Because on Windows the EVT_MENU_CLOSE event is not sent in case of accelerator-keys
        if wx.Platform == '__WXMSW__' and not self.splash:
            self.handleStatusBar(True)

        fname = os.path.join('Res', mtexts.SpecialEdition)
...
# ###########################################
    ...
    def onCampus(self, event):
        ...
        if wx.Platform == '__WXMSW__' and not self.splash:
            self.handleStatusBar(True)
        hframe = htmlhelpframecampus.HtmlHelpFrameCampus(self, -1, mtexts.txts['Morinus'], 'Campus')


    def onLibros(self, event):
        #Because on Windows the EVT_MENU_CLOSE event is not sent in case of accelerator-keys
        if wx.Platform == '__WXMSW__' and not self.splash:
            self.handleStatusBar(True)
        hframe = htmlhelpframecampus.HtmlHelpFrameCampus(self, -1, mtexts.txts['Morinus'], 'Libros')


    def onAprende(self, event):
        #Because on Windows the EVT_MENU_CLOSE event is not sent in case of accelerator-keys
        if wx.Platform == '__WXMSW__' and not self.splash:
            self.handleStatusBar(True)
        hframe = htmlhelpframecampus.HtmlHelpFrameCampus(self, -1, mtexts.txts['Morinus'], 'Aprende')
